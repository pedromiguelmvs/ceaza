...
function listProjects() {
  ...
  for (const item of projects) {
    ...
    const descriptionValue = document.createTextNode(item.description);
...
// gambiarra pra deletar os projetos quando clicar no X
document.body.addEventListener("click", function (event) {
  if (event.target.alt == "deletar") {
    projects.splice(event.path[1].value, 1);
    if (!projects.length) {
      sectionCardsElement.style["height"] = "300px";
      sectionCardsElement.style["display"] = "flex";
      sectionCardsElement.style["align-items"] = "center";
